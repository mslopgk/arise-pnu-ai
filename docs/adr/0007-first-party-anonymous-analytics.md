...
- **과거 로그 백필(2026-07-07)**: 배포 이전 구간(2026-06-18~07-07)은 nginx 접근 로그에서 pageview 5,959건을 백필했다(`meta.bf=1`). 식별자는 (IP,UA,KST일자) 해시 — 솔트 폐기로 역추적 불가, IP 원본 미저장 원칙 유지. 백필 세션은 클릭 계측이 없어 **퍼널 계산에서 제외**되고 페이지별·일별 집계에는 포함된다. 한계: 일자 단위 방문자 근사(일 넘김 방문자 중복 계산), 스크롤·체류·해시 뷰 없음, `/`(게이트웨이)는 응답 크기(406B)로 동일 서버의 서브도메인 정적 사이트와 판별.
